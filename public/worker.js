function calculateOptimality(ticket, maxData) {
    ...
    const durationOptimality =
        1 / (1 + ticket.segments.reduce((total, segment) => total + segment.duration, 0) / maxDuration)
    const stopsCount = ticket.segments.reduce((total, segment) => total + segment.stops.length, 0)
    const stopsOptimality = 1 / (1 + stopsCount / maxStops)
    const totalOptimality = priceOptimality * durationOptimality * stopsOptimality
                        ...
                        const maxData = tickets.reduce(
                            (acc, ticket) => {
                                ...
                                const ticketMaxDuration = Math.max(
                                    ...ticket.segments.map((segment) => segment.duration)
                                )
                                ...
                                acc.maxPrice = Math.max(acc.maxPrice, ticketMaxPrice)
                                acc.maxDuration = Math.max(acc.maxDuration, ticketMaxDuration)
                                acc.maxStops = Math.max(acc.maxStops, ticketMaxStops)
